...
def get_expected_energy_gain(self, region):
    ...
    if v == region]
    energy_pv = self.energy_measurements['pvname'][key[0]]
    factor = energy_measurements['factor'][key[0]]
    if  "L1" in region:
        expected_gain = factor * (energy_pv - self.injector_energy )
    else:
        start_energy_pv = self.energy_measurements['pvname'][key[0]-1]
        expected_gain = factor * ( pvdata[energy] - pvdata[start_energy_pv])
...
def update_energy_gain_cu(self, tao, pvdata, region):
    """Updates Cu Linac energy gain profile based on bending magnets, calculates a fudge and modifies model's cavity amplitudes and phases"""
    expected_gain = self.get_expected_energy_gain(region)
