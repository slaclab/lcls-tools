...
# check where class is being called from?
# Make sure it's only called as a child?
class FitOptions(BaseModel):
    model_config = ConfigDict(
        extra="forbid",
        frozen=True,
    )
    _type:  Optional[str] = 'Gaussian'
    _batch: Optional[bool] = False
    _guess: Optional[dict] = None
    _best_fit: Optional[bool] = False
...
class FitResults(BaseModel):
    ...
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
    #put getter and setters in these classes
     

class Fitting(BaseModel):
    options: SerializeAsAny[Optional[FitOptions]] = None #Change to defult
    results: SerializeAsAny[Optional[FitResults]] = None
    # setter and getter for options
    # just setter for results
    #
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
    ...
    def gaussian(self, data_array):
        """Return Gaussian fit of 1D data array."""
        # Trying to increase consistency of test w/ scipy.norm
        self.results._mean, self.results._std = norm.fit(data_array)
        self.results._curve = norm.pdf(data_array, self.results.mean, self.results.std)
        return 
