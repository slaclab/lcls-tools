...
class BmadModel():
    ...
    def get_rf_quads_pvlist(self, tao):
        ...
        for dm_key in self.all_data_maps.keys():
            ...
            if dm_key in ["cavities", "quad"]:
                elements = (
                    self.all_data_maps[dm_key].data["bmad_name"].to_list())
                ...
                for pv in self.all_data_maps[dm_key].pvlist:
                    if "BEAMCODE" in pv:
                        pv = pv.replace("BEAMCODE2",
                                        f"BEAMCODE{self.beam_code}")
                    pvlist.add(pv)
        if "sc" in self.beam_path:
            pvlist.update(
    ...
    def bdes_to_kmod(self, tao, element, bdes):
        """Returns K1 given BDES"""
        ele = tao.ele_gen_attribs(element)
        bp = ele["E_TOT"] / 1e9 / 299.792458 * 1e4
        # kG m
    ...
    def get_output(self, tao):
        """Returns dictionary of modeled parameters,
        including element name, twiss and Rmats"""
        # Output collecting
        outkeys = [
            "ele.name",
            "ele.ix_ele",
    ...
    def show_twiss(self, tao, element, datum=[]):
        ...
        if not datum == []:
            twiss_datum = tao.data_parameter(datum,
                                             "meas_value")[0].split(";")[1:]
