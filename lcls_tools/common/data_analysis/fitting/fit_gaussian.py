...
def find_widths(xdata, peak_idxs, runs, run_map):
    ...
    for peak_idx in peak_idxs:
        ...
        # the last element of the previous run
            ...
            widths.append(xdata[runs[run_idx + 1][0]] - xdata[runs[run_idx - 1][-1]])
...
def gaussian(x, ctr, wid, amp):
    return amp * exp(-power(x - ctr, 2.0) / (2 * power(wid, 2.0)))
...
def get_guess(xdata, ydata, step, use_zeros, num_peaks):
    ...
    peaks, peak_idx, max_info = (
        get_peaks(ydata, num_peaks, non_zero_runs)
        if not use_zeros
