...
class Measurement(BaseModel, ABC):
    """
    Base abstract class for all meaurements, which serves as the bare minimum skeleton code needed. Should be used only as a parent class
    to all performable measurements.
    ---------------------------
    Arguments:
    ...
    save_location: Optional[DirectoryPath] = None (optional argument that is a path to the directory you wish to save the data)
    ---------------------------
    Methods:
