...
class ScreenBeamProfileMeasurement(Measurement):
    """
    ScreenBeamProfileMeasurement class that allows for background subtraction
        and roi cropping
    ------------------------
    Arguments:
    ...
    device: Screen
    beam_fit_tool: GaussianFit = GaussianFit()  # actually want an additional
    ...
    def single_measure(self) -> dict:
        ...
        processed_image = self.beam_fit_tool.processor.auto_process(raw_image)
        return {"raw_image": raw_image, "processed_image": processed_image}

    def measure(self, n_shots: int = 1) -> dict:
        """
        Measurement function that takes in n_shots as argumentment,
        where n_shots is the number of image profiles
        we would like to measure. Invokes single_measure per shot
        Then if fit_profile = True, fits the x and y projections
        of each image using the provided fitting_tool.
        Results are stored in list of dictionaries where each
        element of the list is results for the image fitting process
