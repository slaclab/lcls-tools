...
def plot_quad_scan_result(emittance_results: EmittanceMeasurementResult):
    ...
    if not isinstance(emittance_results, EmittanceMeasurementResult):
        raise ValueError("emittance_results must be an instance of EmittanceMeasurementResult")
    ...
    ax[0].plot(
        k, 
        np.sqrt(beta[:,0]*emittance_results.emittance[0])*1e3,"--",label="x_fit")
    ax[0].plot(
        k, 
        np.sqrt(beta[:,1]*emittance_results.emittance[1])*1e3,"--",label="y_fit")
