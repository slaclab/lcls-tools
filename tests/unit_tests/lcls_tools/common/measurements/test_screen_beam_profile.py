...
class TestScreenBeamProfileMeasurement(unittest.TestCase):
    ...
    def test_instantiate_pydantic_objects(self):
        ...
        self.screen_test.image = self.create_test_image(size =self.size,center = self.center,radius = self.radius)
        self.screen_beam_profile_measurement = ScreenBeamProfileMeasurement(name = self.control_name,device = self.screen_test, image_processor = self.image_processor, fitting_tool = self.projection_fit)
